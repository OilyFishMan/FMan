...
void state_render(struct state* state)
{
    ...
    for (size_t i = 0; i < state_fs_window_height(state) && i + state->scroll_y < state->pattern_matches_len; ++i) {
...
bool event_normal_mode(struct state* state, const int ch, char* error_message)
{
    switch (ch) {
        ...
        case KEY_LEFT: {
            if (!state_go_back_dir(state, error_message)) {
                return false;
            }
...
void event_typing_mode(struct state* state, const int ch)
{
    switch (ch) {
        ...
        case KEY_BACKSPACE: {
            state_pattern_delete_char(state);
            break;
        }
        ...
        default: {
            state_pattern_add_char(state, ch);
            break;
        }
    }
}
...
int main(void)
{
    ...
    while (state.mode != E_mode_stopped) {
        ...
        if (!state_update(&state, error_message)) {
            exit_success = false;
            goto cleanup_state;
        }
